...
installPhpMyAdmin() {
    ...
    mv phpMyAdmin-*-all-languages/* /var/www/pterodactyl/public/phpmyadmin


    mkdir /var/www/pterodactyl/public/phpmyadmin/tmp
    
    chmod -R 777 /var/www/pterodactyl/public/phpmyadmin/tmp

    rm config.sample.inc.php

    curl -o /var/www/pterodactyl/public/phpmyadmin/config.inc.php $GitHub_Account/config.inc.php
    sed -i -e "s@<blowfish_secret>@${blowfish_secret}@g" /var/www/pterodactyl/public/phpmyadmin/config.inc.php
...
if [ $choice == "5" ]
then
    rm -rf /var/pterodactyl/public/phpmyadmin
    clear
    echo ""
...
# This is an open source script developed specifically for our customers to install Pterodactyl quickly and safely.

# We are not responsible for any type of injection into the script outside the official repository: https://github.com/Sebxs404/Pterodactyl
